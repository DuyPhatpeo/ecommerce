...
const Toggle: React.FC<ToggleProps> = ({
  ...
  return (
    <div className="flex items-center gap-2 select-none">
      <button
        onClick={onChange}
        className={`relative w-11 h-6 rounded-none transition-colors duration-300 ${
          checked ? activeBg : inactiveBg
        }`}
      >
        <span
          className={`absolute top-0.5 left-0.5 h-5 w-5 bg-white rounded-none shadow transition-transform duration-300 ${
